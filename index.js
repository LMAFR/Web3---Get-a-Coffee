import {
  ...
  async function showBakeryBalance() {
    try {
      ...
      const balanceWei = await client.getBalance({ address: contractAddress });
      const balanceEth = formatEther(balanceWei);
      toastInfo(`Contract balance (${shortenHex(contractAddress, { start: 4, end: 4 })}): ${balanceEth} ETH`);
    } catch (err) {
      console.error(err);
      toastWarn('Failed to fetch contract balance. Is Anvil running at http://127.0.0.1:8545?');
    }
  }

  async function getBalanceCopied() {
  ...
  console.log('Current chain:', await getCurrentChain(client));
      ...
      const { request } = await client.simulateContract({
        address: contractAddress,
        abi,
        chain: await getCurrentChain(client), //ain: await getCurrentChain(client), // fou chain IDdry chain ID
        functionName: 'fund',
